...
namespace WizBot
{
    ...
    {
        public static WizStats Instance { get; } = new WizStats();
        ...
        private WizStats()
        {
            ...
            ServerCount = WizBot.Client.Servers.Count();
            var channels = WizBot.Client.Servers.SelectMany(s => s.AllChannels);
            ...
            WizBot.Client.MessageReceived += (s, e) => messageCounter++;

            WizBot.Client.JoinedServer += (s, e) =>
            {
                try
                {
                    ServerCount++;
            ...
            WizBot.Client.ChannelCreated += (s, e) =>
            {
                try
                {
            ...
            WizBot.Client.ChannelDestroyed += (s, e) =>
            {
                try
                {
                    if (e.Channel.IsPrivate)
        ...
        {
            if (string.IsNullOrWhiteSpace(WizBot.Creds.CarbonKey))
                return;
            try
            {
                using (var content = new FormUrlEncodedContent(new Dictionary<string, string> {
                                { "servercount", WizBot.Client.Servers.Count().ToString() },
                                { "key", WizBot.Creds.CarbonKey }
                    }))
                {
        ...
        public Task LoadStats() =>
            Task.Run(() =>
            {
                ...
                sb.AppendLine("`Author: Kwoth & Wizkiller96` `Library: Discord.Net`");
                sb.AppendLine($"`Bot Version: {BotVersion}`");
                sb.AppendLine($"`Bot id: {WizBot.Client.CurrentUser.Id}`");
                sb.Append("`Owners' Ids:` ");
                sb.AppendLine("`" + String.Join(", ", WizBot.Creds.OwnerIds) + "`");
                ...
                sb.AppendLine($"`Message queue size: {WizBot.Client.MessageQueue.Count}`");
        ...
        private async Task StartCollecting()
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    var realOnlineUsers = await Task.Run(() => WizBot.Client.Servers
