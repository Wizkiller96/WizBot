...
namespace WizBot.Modules.Gambling.Commands
{
    class AnimalRacing : DiscordCommand
    {
        ...
        public class AnimalRace
        {
            ...
            private ConcurrentQueue<string> animals = new ConcurrentQueue<string>(WizBot.Config.RaceAnimals.Shuffle());
            ...
            public AnimalRace(ulong serverId, Channel ch)
            {
                ...
                Task.Run(async () =>
                {
                    try
                    {
                        await raceChannel.SendMessage($"🏁`Race is starting in 20 seconds or when the room is full. Type {WizBot.Config.CommandPrefixes.Gambling}jr to join the race.`");
...
|🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🔚|";
                ...
                if (winner.AmountBet > 0)
                {
                    ...
                    await raceChannel.SendMessage($"🏁 {winner.User.Mention} as {winner.Animal} **Won the race and {wonAmount}{WizBot.Config.CurrencySign}!**");
                }
                else
                {
            ...
            public async Task<bool> JoinRace(User u, int amount = 0)
            {
                ...
                participants.Add(p);
                await raceChannel.SendMessage($"{u.Mention} **joined the race as a {p.Animal}" + (amount > 0 ? $" and bet {amount} {WizBot.Config.CurrencyName.SnPl(amount)}!**" : "**"));
                return true;
            }
        }
