...
namespace WizBot.Modules.Utility.Commands
{
    class Remind : DiscordCommand
    {
        ...
        public Remind(DiscordModule module) : base(module)
        {
            ...
            WizBot.OnReady += () => reminders = remList.Select(StartNewReminder).ToList();
        }
        ...
        private Timer StartNewReminder(Reminder r)
        {
            ...
            t.Elapsed += async (s, e) =>
            {
                try
                {
                    ...
                    if (r.IsPrivate)
                    {
                        ch = WizBot.Client.PrivateChannels.FirstOrDefault(c => (long)c.Id == r.ChannelId);
                        if (ch == null)
                            ch = await WizBot.Client.CreatePrivateChannel((ulong)r.ChannelId).ConfigureAwait(false);
                    }
                    else
                        ch = WizBot.Client.GetServer((ulong)r.ServerId)?.GetChannel((ulong)r.ChannelId);

                    if (ch == null)
                        return;

                    await ch.SendMessage(
                        replacements.Aggregate(WizBot.Config.RemindMessageFormat,
