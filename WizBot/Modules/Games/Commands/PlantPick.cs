            ...
            try
            {
                ...
                if (config.GenerateCurrencyChannels.TryGetValue(e.Channel.Id, out cd))
                    if (!plantpickCooldowns.TryGetValue(e.Channel.Id, out lastSpawned) || (lastSpawned + new TimeSpan(0, cd, 0)) < now)
                    {
                        ...
                        if (rnd == 0)
                        {
                            var msgs = new[] { await e.Channel.SendFile(GetRandomCurrencyImagePath()), await e.Channel.SendMessage($"❗ A random {WizBot.Config.CurrencyName} appeared! Pick it up by typing `>pick`") };
        ...
        internal override void Init(CommandGroupBuilder cgb)
        {
            cgb.CreateCommand(Module.Prefix + "pick")
                ...
                .Do(async e =>
                {
                    ...
                    var msg = await e.Channel.SendMessage($"**{e.User.Name}** picked a {WizBot.Config.CurrencyName}!").ConfigureAwait(false);
            ...
            cgb.CreateCommand(Module.Prefix + "plant")
                ...
                .Do(async e =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            await e.Channel.SendMessage($"There is already a {WizBot.Config.CurrencyName} in this channel.").ConfigureAwait(false);
                            return;
                        }
                        var removed = await FlowersHandler.RemoveFlowers(e.User, "Planted a flower.", 1, true).ConfigureAwait(false);
                        if (!removed)
                        {
                            await e.Channel.SendMessage($"You don't have any {WizBot.Config.CurrencyName}s.").ConfigureAwait(false);
                            return;
                        }
                        ...
                        Message msg;
                        if (file == null)
                            msg = await e.Channel.SendMessage(WizBot.Config.CurrencySign).ConfigureAwait(false);
                        else
                            msg = await e.Channel.SendFile(file).ConfigureAwait(false);
                        var vowelFirst = new[] { 'a', 'e', 'i', 'o', 'u' }.Contains(WizBot.Config.CurrencyName[0]);
                        var msg2 = await e.Channel.SendMessage($"Oh how Nice! **{e.User.Name}** planted {(vowelFirst ? "an" : "a")} {WizBot.Config.CurrencyName}. Pick it using {Module.Prefix}pick").ConfigureAwait(false);
            ...
            cgb.CreateCommand(Prefix + "gencurrency")
                .Alias(Prefix + "gc")
                .Description($"Toggles currency generation on this channel. Every posted message will have 2% chance to spawn a {WizBot.Config.CurrencyName}. Optional parameter cooldown time in minutes, 5 minutes by default. Requires Manage Messages permission. | `{Prefix}gc` or `{Prefix}gc 60`")
