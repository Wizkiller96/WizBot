...
namespace WizBot.Modules.Games
{
    internal class GamesModule : DiscordModule
    {
        ...
        public override string Prefix { get; } = WizBot.Config.CommandPrefixes.Games;
            ...
            manager.CreateCommands("", cgb =>
            {
                ...
                cgb.CreateCommand(Prefix + "rps")
                    ...
                    .Do(async e =>
                    {
                        ...
                        var wizPick = new Random().Next(0, 3);
                        var msg = "";
                        if (pick == wizPick)
                            msg = $"It's a draw! Both picked :{GetRPSPick(pick)}:";
                        else if ((pick == 0 && wizPick == 1) ||
                                 (pick == 1 && wizPick == 2) ||
                                 (pick == 2 && wizPick == 0))
                            msg = $"{WizBot.BotMention} won! :{GetRPSPick(wizPick)}: beats :{GetRPSPick(pick)}:";
                        else
                            msg = $"{e.User.Mention} won! :{GetRPSPick(pick)}: beats :{GetRPSPick(wizPick)}:";
