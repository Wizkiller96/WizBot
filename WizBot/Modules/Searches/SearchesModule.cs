...
using WizBot.Modules.Searches.Commands;
using WizBot.Modules.Searches.Commands.IMDB;
...
🌄 **Sunrise:** {obj["sunrise"]} 🌇 **Sunset:** {obj["sunset"]}").ConfigureAwait(false);
                ...
                cgb.CreateCommand(Prefix + "i")
                   ...
                   .Parameter("query", ParameterType.Unparsed)
                       .Do(async e =>
                       {
                           ...
                           try
                           {
                               var reqString = $"https://www.googleapis.com/customsearch/v1?q={Uri.EscapeDataString(e.GetArg("query"))}&cx=018084019232060951019%3Ahs5piey28-e&num=1&searchType=image&fields=items%2Flink&key={WizBot.Creds.GoogleAPIKey}";
                ...
                cgb.CreateCommand(Prefix + "ir")
                   ...
                   .Parameter("query", ParameterType.Unparsed)
                       .Do(async e =>
                       {
                           ...
                           try
                           {
                               var reqString = $"https://www.googleapis.com/customsearch/v1?q={Uri.EscapeDataString(e.GetArg("query"))}&cx=018084019232060951019%3Ahs5piey28-e&num=1&searchType=image&start={ rng.Next(1, 150) }&fields=items%2Flink&key={WizBot.Creds.GoogleAPIKey}";
                               var obj = JObject.Parse(await SearchHelper.GetResponseStringAsync(reqString).ConfigureAwait(false));
                               await e.Channel.SendMessage(obj["items"][0]["link"].ToString()).ConfigureAwait(false);
                           }
                           catch (HttpRequestException exception)
                ...
                cgb.CreateCommand(Prefix + "hs")
                  ...
                  .Do(async e =>
                  {
                      ...
                      await e.Channel.SendIsTyping().ConfigureAwait(false);
                      var headers = new Dictionary<string, string> { { "X-Mashape-Key", WizBot.Creds.MashapeKey } };
