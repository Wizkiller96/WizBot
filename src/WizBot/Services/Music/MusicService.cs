...
using WizBot.Services.Music.SongResolver;
...
namespace WizBot.Services.Music
{
    public class MusicService : INService
    {
        ...
        public async Task TryQueueRelatedSongAsync(SongInfo song, ITextChannel txtCh, IVoiceChannel vch)
        {
            ...
            var si = await ResolveSong(related[new WizBotRandom().Next(related.Length)], _client.CurrentUser.ToString(), MusicType.YouTube);
            if (si == null)
