...
using WizBot.Modules.CustomReactions;
...
namespace WizBot.Services
{
    ...
    public class CommandHandler
    {
        ...
        public Task StartHandling()
        {
            var _ = Task.Run(async () =>
            {
                ...
                if (!ownerChannels.Any())
                    ...
                    _log.Info($"Created {ownerChannels.Length} out of {WizBot.Credentials.OwnerIds.Length} owner message channels.");
            });
        ...
        {
            var hs = new HashSet<ulong>(WizBot.Credentials.OwnerIds);
            ...
            ownerChannels = channels.OrderBy(x => WizBot.Credentials.OwnerIds.IndexOf(x.Key))
                    .Select(x => x.Value)
                    .ToImmutableArray();
        }
        private async Task<bool> TryRunCleverbot(IUserMessage usrMsg, SocketGuild guild)
        {
            ...
            try
            {
                ...
                if (!pc.Permissions.CheckPermissions(usrMsg,
                    WizBot.ModulePrefixes[typeof(Games).Name] + "cleverbot",
                    typeof(Games).Name,
                    out int index))
                {
        ...
        private Task MessageReceivedHandler(SocketMessage msg)
        {
            var _ = Task.Run(async () =>
            {
                try
                {
                    if (msg.Author.IsBot || !WizBot.Ready) //no bots, wait until bot connected and initialized
                        return;
