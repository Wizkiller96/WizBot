...
public partial class ResponseBuilder
{
    public class PaginationSender<T>
    {
        ...
        private WizButtonInteractionHandler? left;
        private WizButtonInteractionHandler? right;
        private WizInteractionBase? extra;
        ...
        public async Task SendAsync(bool ephemeral = false)
        {
            ...
            WizInteractionBase? maybeInter = null;

            var model = await _builder.BuildAsync(ephemeral);

            async Task<(WizButtonInteractionHandler left, WizInteractionBase? extra, WizButtonInteractionHandler right)>
                GetInteractions()
            {
                ...
                var leftBtnInter = new WizButtonInteractionHandler(_client,
                    model.User?.Id ?? 0,
                    leftButton,
                    (smc) =>
                    {
                ...
                var rightBtnInter = new WizButtonInteractionHandler(_client,
                    model.User?.Id ?? 0,
                    rightButton,
                    (smc) =>
                    {
