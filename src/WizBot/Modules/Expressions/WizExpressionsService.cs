...
public sealed class WizExpressionsService : IExecOnMessage, IReadyExecutor, INService
{
    ...
    private readonly TypedKey<WizExpression> _gexprAddedKey = new("gexpr.added");
    private readonly TypedKey<int> _gexprDeletedkey = new("gexpr.deleted");
    private readonly TypedKey<WizExpression> _gexprEditedKey = new("gexpr.edited");
    ...
    private WizExpression[] globalExpressions = Array.Empty<WizExpression>();
    private ConcurrentDictionary<ulong, WizExpression[]> newguildExpressions = new();
    ...
    private async Task ReloadInternal()
    {
        ...
        var guildItems = await uow.GetTable<WizExpression>()
                                  .AsNoTracking()
        ...
        {
            var globalItems = uow.Set<WizExpression>()
                                 .AsNoTracking()
    ...
    private WizExpression TryGetExpression(IUserMessage umsg)
    {
        if (!_isReady)
            return null;
    ...
    private WizExpression MatchExpressions(in ReadOnlySpan<char> content, WizExpression[] exprs)
    {
        var result = new List<WizExpression>(1);
    ...
    public async Task ResetExprReactions(ulong? maybeGuildId, int id)
    {
        WizExpression expr;
        await using var uow = _db.GetDbContext();
        expr = uow.Set<WizExpression>().GetById(id);
        if (expr is null)
            return;
    ...
    private Task UpdateInternalAsync(ulong? maybeGuildId, WizExpression expr)
    {
    ...
    private void UpdateInternal(ulong? maybeGuildId, WizExpression expr)
    {
        if (maybeGuildId is { } guildId)
        {
    ...
    private Task AddInternalAsync(ulong? maybeGuildId, WizExpression expr)
    {
    ...
    private WizExpression[] DeleteInternal(
        IReadOnlyList<WizExpression> exprs,
        int id,
        out WizExpression deleted)
    {
        deleted = null;
        if (exprs is null || exprs.Count == 0)
            return exprs as WizExpression[] ?? exprs?.ToArray();

        var newExprs = new WizExpression[exprs.Count - 1];
    ...
    public WizExpression GetExpression(ulong? guildId, int id)
    {
        using var uow = _db.GetDbContext();
        var expr = uow.Set<WizExpression>().GetById(id);
        if (expr is null || expr.GuildId != guildId)
            return null;
    ...
    {
        using var uow = _db.GetDbContext();
        var count = uow.Set<WizExpression>().ClearFromGuild(guildId);
        uow.SaveChanges();
    ...
    private ValueTask OnGexprAdded(WizExpression c)
    {
        lock (_gexprWriteLock)
        {
            var newGlobalReactions = new WizExpression[globalExpressions.Length + 1];
    ...
    private ValueTask OnGexprEdited(WizExpression c)
    {
        lock (_gexprWriteLock)
        {
    ...
    public async Task<WizExpression> AddAsync(
        ulong? guildId,
        string key,
        string message,
        bool ca = false,
        ...
        bool dm = false)
    {
        ...
        await using (var uow = _db.GetDbContext())
        {
            uow.Set<WizExpression>().Add(expr);
            await uow.SaveChangesAsync();
        }
    ...
    public async Task<WizExpression> EditAsync(
        ulong? guildId,
        int id,
        string message,
        bool? ca = null,
    ...
    public async Task<WizExpression> DeleteAsync(ulong? guildId, int id)
    {
        await using var uow = _db.GetDbContext();
        var toDelete = uow.Set<WizExpression>().GetById(id);
        ...
        {
            uow.Set<WizExpression>().Remove(toDelete);
            await uow.SaveChangesAsync();
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public WizExpression[] GetExpressionsFor(ulong? maybeGuildId)
    {
        if (maybeGuildId is { } guildId)
            return newguildExpressions.TryGetValue(guildId, out var exprs) ? exprs : Array.Empty<WizExpression>();
    ...
    public async Task<(IReadOnlyCollection<WizExpression> Exprs, int TotalCount)> FindExpressionsAsync(
        ulong guildId,
        string query,
        int page)
    {
