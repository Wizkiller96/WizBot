...
public sealed class WizBotExpressionsService : IExecOnMessage, IReadyExecutor
{
    ...
    private async Task ReloadInternal(IReadOnlyList<ulong> allGuildIds)
    {
        ...
        var guildItems = await uow.Set<WizBotExpression>()
                                  .AsNoTracking()
        ...
        {
            var globalItems = uow.Set<WizBotExpression>()
                                 .AsNoTracking()
    ...
    public async Task ResetExprReactions(ulong? maybeGuildId, int id)
    {
        ...
        await using var uow = _db.GetDbContext();
        expr = uow.Set<WizBotExpression>().GetById(id);
        if (expr is null)
            return;
    ...
    public WizBotExpression GetExpression(ulong? guildId, int id)
    {
        ...
        var expr = uow.Set<WizBotExpression>().GetById(id);
        if (expr is null || expr.GuildId != guildId)
            return null;
    ...
    {
        using var uow = _db.GetDbContext();
        var count = uow.Set<WizBotExpression>().ClearFromGuild(guildId);
        uow.SaveChanges();
    ...
    public async Task<bool> ImportExpressionsAsync(ulong? guildId, string input)
    {
        ...
        foreach (var entry in data)
        {
            ...
            await uow.Set<WizBotExpression>()
    ...
    {
        await using var uow = _db.GetDbContext();
        var exprs = await uow.Set<WizBotExpression>().AsNoTracking().Where(x => x.GuildId == gc.GuildId).ToArrayAsync();
    ...
    public async Task<WizBotExpression> AddAsync(
        ulong? guildId,
        string key,
        string message,
        bool ca = false,
        ...
        bool dm = false)
    {
        ...
        await using (var uow = _db.GetDbContext())
        {
            uow.Set<WizBotExpression>().Add(expr);
            await uow.SaveChangesAsync();
        }
    ...
    public async Task<WizBotExpression> EditAsync(
        ulong? guildId,
        int id,
        string message,
        bool? ca = null,
    ...
    {
        await using var uow = _db.GetDbContext();
        var toDelete = uow.Set<WizBotExpression>().GetById(id);
        ...
        {
            uow.Set<WizBotExpression>().Remove(toDelete);
            await uow.SaveChangesAsync();
    ...
    public async Task<(IReadOnlyCollection<WizBotExpression> Exprs, int TotalCount)> FindExpressionsAsync(
        ulong guildId,
        string query,
        int page)
    {
