...
";
        ...
        public async Task Rps(RpsPick pick, ShmartNumber amount = default)
        {
                     ...
                     (pick == RpsPick.Scissors && wizbotPick == RpsPick.Paper))
            {
                ...
                msg = GetText(strs.rps_win(ctx.User.Mention, getRpsPick(pick), getRpsPick(wizbotPick)));
            }
            else
            {
                embed.WithErrorColor();
                amount = 0;
                msg = GetText(strs.rps_win(ctx.Client.CurrentUser.Mention, getRpsPick(wizbotPick), getRpsPick(pick)));
            }
