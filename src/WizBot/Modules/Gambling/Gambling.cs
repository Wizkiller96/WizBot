...
//                var generator = new WizBotRandom();
...
//        }
        ...
        public async Task BetRoll(long amount)
        {
            ...
            var rng = new WizBotRandom().Next(0, 101);
            ...
            {
                str += $"Congratulations! You won {amount * WizBot.BotConfig.Betroll67Multiplier}{CurrencySign} for rolling above 66";
                await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble", (int)(amount * WizBot.BotConfig.Betroll67Multiplier), false).ConfigureAwait(false);
            }
            else if (rng < 100)
            {
                str += $"Congratulations! You won {amount * WizBot.BotConfig.Betroll91Multiplier}{CurrencySign} for rolling above 90.";
                await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble", (int)(amount * WizBot.BotConfig.Betroll91Multiplier), false).ConfigureAwait(false);
            }
            else
            {
                str += $"👑 Congratulations! You won {amount * WizBot.BotConfig.Betroll100Multiplier}{CurrencySign} for rolling **100**. 👑";
                await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble", (int)(amount * WizBot.BotConfig.Betroll100Multiplier), false).ConfigureAwait(false);
            }
        ...
        public async Task Leaderboard()
        {
            ...
            var embed = new EmbedBuilder()
                .WithOkColor()
                .WithTitle(WizBot.BotConfig.CurrencySign + " Leaderboard");
            ...
            for (var i = 0; i < richest.Count; i++)
            {
                ...
                embed.AddField(efb => efb.WithName("#" + (i + 1) + " " + usrStr).WithValue(x.Amount.ToString() + " " + WizBot.BotConfig.CurrencySign).WithIsInline(true));
            }
