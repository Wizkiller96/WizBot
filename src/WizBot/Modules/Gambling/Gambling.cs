...
namespace WizBot.Modules.Gambling
{
    ...
    public partial class Gambling : WizBotTopLevelModule
    {
        ...
        public async Task BetRoll(long amount)
        {
            ...
            else
            {
                if (rnd < 91)
                {
                    str += GetText("br_win", (amount * WizBot.BotConfig.Betroll67Multiplier) + CurrencySign, 66);
                    await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble",
                        (int)(amount * WizBot.BotConfig.Betroll67Multiplier), false).ConfigureAwait(false);
                }
                else if (rnd < 100)
                {
                    str += GetText("br_win", (amount * WizBot.BotConfig.Betroll91Multiplier) + CurrencySign, 90);
                    await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble",
                        (int)(amount * WizBot.BotConfig.Betroll91Multiplier), false).ConfigureAwait(false);
                }
                else
                {
                    str += GetText("br_win", (amount * WizBot.BotConfig.Betroll100Multiplier) + CurrencySign, 100) + " 👑";
                    await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble",
        ...
        public async Task Leaderboard()
        {
            ...
            var embed = new EmbedBuilder()
                .WithOkColor()
                .WithTitle(WizBot.BotConfig.CurrencySign + " " + GetText("leaderboard"));
