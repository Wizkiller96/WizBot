...
namespace WizBot.Modules.Gambling
{
    ...
    {
        [Group]
        public class FlowerShop : WizBotSubmodule
        {
            public enum Role
            {
                Role
            ...
            public async Task Shop(int page = 1)
            {
                ...
                await Context.Channel.SendPaginatedConfirmAsync(page + 1, (curPage) =>
                {
                    ...
                    var embed = new EmbedBuilder().WithOkColor()
                        .WithTitle(GetText("shop", WizBot.BotConfig.CurrencySign));

                    for (int i = 0; i < entries.Count; i++)
                    {
                        var entry = entries[i];
                        embed.AddField(efb => efb.WithName($"#{i + 1} - {entry.Price}{WizBot.BotConfig.CurrencySign}").WithValue(EntryToString(entry)).WithIsInline(true));
                    }
                    return embed;
