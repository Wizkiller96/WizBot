    ...
    public partial class Gambling
    {
        ...
        public class WaifuClaimCommands : WizBotSubmodule
        {
            ...
            public async Task WaifuClaim(int amount, [Remainder]IUser target)
            {
                if (amount < 50)
                {
                    await Context.Channel.SendErrorAsync($"{Context.User.Mention} No waifu is that cheap. You must pay at least 50{WizBot.BotConfig.CurrencySign} to get a waifu, even if their actual value is lower.").ConfigureAwait(false);
                    return;
                }
                ...
                {
                    await Context.Channel.SendConfirmAsync($"{Context.User.Mention} you don't have {amount}{WizBot.BotConfig.CurrencySign}!")
                            .ConfigureAwait(false);
                }
                else
                {
                    var msg = $"{Context.User.Mention} claimed {target.Mention} as their waifu for {amount}{WizBot.BotConfig.CurrencySign}!";
                    if (w.Affinity?.UserId == Context.User.Id)
                        msg += $"\n🎉 Their love is fulfilled! 🎉\n**{target}'s** new value is {w.Price}{WizBot.BotConfig.CurrencySign}!";
            ...
            public async Task Divorce([Remainder]IUser target)
            {
                ...
                {
                    await Context.Channel.SendConfirmAsync($"{Context.User.Mention} You have divorced a waifu who likes you. You heartless monster.\n{w.Waifu} received {amount}{WizBot.BotConfig.CurrencySign} as a compensation.").ConfigureAwait(false);
                }
                else if (result == DivorceResult.Success)
                {
                    await Context.Channel.SendConfirmAsync($"{Context.User.Mention} You have divorced a waifu who doesn't like you. You received {amount}{WizBot.BotConfig.CurrencySign} back.").ConfigureAwait(false);
                }
            ...
            public async Task WaifuLeaderboard()
            {
                ...
                for (int i = 0; i < waifus.Count; i++)
                {
                    ...
                    embed.AddField(efb => efb.WithName("#" + (i + 1) + " - " + w.Price + WizBot.BotConfig.CurrencySign).WithValue(w.ToString()).WithIsInline(false));
                }
