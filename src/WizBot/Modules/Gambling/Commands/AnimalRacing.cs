                        ...
                        try
                        {
                            try
                            {
                                await raceChannel.SendConfirmAsync("Animal Race", $"Starting in 20 seconds or when the room is full.",
                                    footer: $"Type {WizBot.ModulePrefixes[typeof(Gambling).Name]}jr to join the race.");
                            }
                            catch (Exception ex)
                            {
                                _log.Warn(ex);
...
|🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🏁🔚|";
                ...
                public async Task JoinRace(IGuildUser u, int amount = 0)
                {
                    ...
                    if (amount > 0)
                        ...
                        {
                            try { await raceChannel.SendErrorAsync($"{u.Mention} You don't have enough {WizBot.BotConfig.CurrencyPluralName}.").ConfigureAwait(false); } catch { }
                            return;
                        }
                    participants.Add(p);
