...
public class SearchesService : INService
{
    ...
    {
        var data = await _cache.GetOrAddCachedDataAsync($"wizbot_rip_{text}_{imgUrl}",
    ...
    public string GetRandomImageUrl(ImageTag tag)
    {
        ...
        return $"https://wizbot-pictures.nyc3.digitaloceanspaces.com/{subpath}/"
    ...
    public Task<OmdbMovie> GetMovieDataAsync(string name)
    {
        ...
        return _cache.GetOrAddCachedDataAsync($"wizbot_movie_{name}", GetMovieDataFactory, name, TimeSpan.FromDays(1));
    }
