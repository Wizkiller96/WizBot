...
namespace WizBot.Modules.Searches
{
    ...
    public partial class Searches : WizBotTopLevelModule
    {
        ...
        public async Task Youtube([Remainder] string query = null)
        {
            ...
            //await Context.Channel.EmbedAsync(new Discord.API.Embed() { Video = new Discord.API.EmbedVideo() { Url = result.Replace("watch?v=", "embed/") }, Color = WizBot.OkColor }).ConfigureAwait(false);
        }
        ...
        public async Task Image([Remainder] string terms = null)
        {
            ...
            try
            {
                var res = await WizBot.Google.GetImageAsync(terms).ConfigureAwait(false);
                var embed = new EmbedBuilder()
                    .WithOkColor()
        ...
        public async Task RandomImage([Remainder] string terms = null)
        {
            ...
            try
            {
                var res = await WizBot.Google.GetImageAsync(terms, new WizBotRandom().Next(0, 50)).ConfigureAwait(false);
                var embed = new EmbedBuilder()
                    .WithOkColor()
                    .WithAuthor(eab => eab.WithName("Image Search For: " + terms.TrimTo(50))
                        .WithUrl("https://www.google.rs/search?q=" + terms + "&source=lnms&tbm=isch")
        ...
        public async Task Lmgtfy([Remainder] string ffs = null)
        {
            ...
            await Context.Channel.SendConfirmAsync(await WizBot.Google.ShortenUrl($"<http://lmgtfy.com/?q={ Uri.EscapeUriString(ffs) }>"))
