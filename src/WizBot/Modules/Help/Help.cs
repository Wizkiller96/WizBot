...
namespace WizBot.Modules.Help
{
    public class Help : WizBotTopLevelModule
    {
        ...
        public string HelpString => String.Format(_config.HelpString, _creds.ClientId, WizBot.Prefix);
        ...
        public async Task Hgit()
        {
            ...
            foreach (var com in _cmds.Commands.OrderBy(com => com.Module.GetTopLevelModule().Name).GroupBy(c => c.Aliases.First()).Select(g => g.First()))
            {
                ...
                helpstr.AppendLine($"{string.Join(" ", com.Aliases.Select(a => "`" + a + "`"))} |" +
                                   $" {string.Format(com.Summary, WizBot.Prefix)} {GetCommandRequirements(com)} |" +
                                   $" {string.Format(com.Remarks, WizBot.Prefix)}");
            }
