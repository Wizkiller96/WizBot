    ...
    public partial class Administration
    {
        ...
        public class ServerGreetCommands : ModuleBase
        {
            ...
            static ServerGreetCommands()
            {
                ...
                GuildConfigsCache = new ConcurrentDictionary<ulong, GreetSettings>(WizBot.AllGuildConfigs.ToDictionary(g => g.GuildId, (g) => GreetSettings.Create(g)));
            }
            ...
            public async Task GreetDmMsg([Remainder] string text = null)
            {
                ...
                if (!sendGreetEnabled)
                    await Context.Channel.SendConfirmAsync($"ℹ️ Enable DM greet messsages by typing `{WizBot.ModulePrefixes[typeof(Administration).Name]}greetdm`").ConfigureAwait(false);
            }
            ...
            public async Task ByeMsg([Remainder] string text = null)
            {
                ...
                if (!sendByeEnabled)
                    await Context.Channel.SendConfirmAsync($"ℹ️ Enable bye messsages by typing `{WizBot.ModulePrefixes[typeof(Administration).Name]}bye`").ConfigureAwait(false);
            }
