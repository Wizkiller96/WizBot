...
{Format.Code(GetText("command_text"))}: {x.CommandText}";
            ...
            public async Task Leave([Remainder] string guildStr)
            {
                ...
                var server = WizBot.Client.Guilds.FirstOrDefault(g => g.Id.ToString() == guildStr) ??
                    WizBot.Client.Guilds.FirstOrDefault(g => g.Name.Trim().ToUpperInvariant() == guildStr);
            ...
            public async Task Send(string where, [Remainder] string msg = null)
            {
                ...
                var server = WizBot.Client.Guilds.FirstOrDefault(s => s.Id == sid);
            ...
            {
                var channels = WizBot.Client.Guilds.Select(g => g.DefaultChannel).ToArray();
                if (channels == null)
                    return;
