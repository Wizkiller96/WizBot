...
namespace WizBot.Modules.Administration
{
    public partial class Administration
    {
        ...
        public class VcRoleCommands : WizBotSubmodule
        {
            ...
            {
                WizBot.Client.UserVoiceStateUpdated += ClientOnUserVoiceStateUpdated;
                vcRoles = new ConcurrentDictionary<ulong, ConcurrentDictionary<ulong, IRole>>();
                foreach (var gconf in WizBot.AllGuildConfigs)
                {
                    var g = WizBot.Client.GetGuild(gconf.GuildId);
                    if (g == null)
