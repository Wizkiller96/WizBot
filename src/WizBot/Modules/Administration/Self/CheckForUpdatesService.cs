...
public sealed class CheckForUpdatesService : INService, IReadyExecutor
{
    ...
    public async Task OnReadyAsync()
    {
        ...
        while (await timer.WaitForNextTickAsync())
        {
            ...
            try
            {
                const string URL = "https://cdn.wizbot.cc/cmds/versions.json";
                ...
                if (latestVersion > lastKnownVersion)
                {
                    ...
                    // pull changelog
                    var changelog = await http.GetStringAsync("https://gitlab.com/WizNet/WizBot/-/raw/v4/CHANGELOG.md");
                    ...
                    await creds.OwnerIds
                        .Select(async x =>
                        {
                            ...
                            var eb = _ebs.Create()
                                .WithOkColor()
                                .WithAuthor($"WizBot v{latestVersion} Released!")
                                .WithTitle("Changelog")
                                .WithUrl("https://gitlab.com/WizNet/WizBot/-/blob/v4/CHANGELOG.md")
