...
";
    ...
    private WizBotExpression[] globalReactions;
    private ConcurrentDictionary<ulong, WizBotExpression[]> newGuildReactions;
    ...
    private ValueTask OnGexprAdded(WizBotExpression c)
    {
        lock (_gexprWriteLock)
        {
            var newGlobalReactions = new WizBotExpression[globalReactions.Length + 1];
    ...
    {
        if (maybeGuildId is { } guildId)
            return newGuildReactions.TryGetValue(guildId, out var exprs) ? exprs : Array.Empty<WizBotExpression>();
