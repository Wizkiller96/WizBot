...
namespace WizBot.Modules.CustomReactions
{
    ...
    public class CustomReactions : DiscordModule
    {
        ...
        public async Task AddCustReact(IUserMessage imsg, string key, [Remainder] string message)
        {
            ...
            if ((channel == null && !WizBot.Credentials.IsOwner(imsg.Author)) || (channel != null && !((IGuildUser)imsg.Author).GuildPermissions.Administrator))
            {
            ...
            await imsg.Channel.EmbedAsync(new EmbedBuilder().WithColor(WizBot.OkColor)
