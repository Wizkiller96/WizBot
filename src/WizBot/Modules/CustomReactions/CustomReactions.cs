...
namespace WizBot.Modules.CustomReactions
{
    ...
    public class CustomReactions : WizBotTopLevelModule
    {
        ...
        public async Task AddCustReact(string key, [Remainder] string message)
        {
            ...
            if ((channel == null && !WizBot.Credentials.IsOwner(Context.User)) || (channel != null && !((IGuildUser)Context.User).GuildPermissions.Administrator))
            {
        ...
        {
            if ((Context.Guild == null && !WizBot.Credentials.IsOwner(Context.User)) || (Context.Guild != null && !((IGuildUser)Context.User).GuildPermissions.Administrator))
            {
                await ReplyErrorLocalized("insuff_perms").ConfigureAwait(false);
                return;
            }
        ...
        {
            if ((Context.Guild == null && !WizBot.Credentials.IsOwner(Context.User)) ||
