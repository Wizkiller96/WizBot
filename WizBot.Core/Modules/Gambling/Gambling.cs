...
namespace WizBot.Modules.Gambling
{
    public partial class Gambling : GamblingTopLevelModule<GamblingService>
    {
        ...
        public async Task Timely()
        {
            ...
            await ReplyConfirmLocalized("timely", n(val) + Bc.BotConfig.CurrencySign, period).ConfigureAwait(false);
        }
        ...
        public async Task TimelySet(int num, int period = 24)
        {
            ...
            if (num == 0)
                ...
                await ReplyConfirmLocalized("timely_set", Format.Bold(n(num) + Bc.BotConfig.CurrencySign), Format.Bold(period.ToString())).ConfigureAwait(false);
        }
        ...
        public async Task Give(ShmartNumber amount, IGuildUser receiver, [Remainder] string msg = null)
        {
            ...
            await ReplyConfirmLocalized("gifted", n(amount) + CurrencySign, Format.Bold(receiver.ToString()), msg)
        ...
        public async Task Award(ShmartNumber amount, ulong usrId, [Remainder] string msg = null)
        {
            ...
            await ReplyConfirmLocalized("awarded", n(amount) + CurrencySign, $"<@{usrId}>").ConfigureAwait(false);
        }
        ...
        public async Task Take(ShmartNumber amount, [Remainder] IGuildUser user)
        {
            ...
            if (await _cs.RemoveAsync(user, $"Taken by bot staff. ({Context.User.Username}/{Context.User.Id})", amount,
                ...
                await ReplyConfirmLocalized("take", n(amount) + CurrencySign, Format.Bold(user.ToString())).ConfigureAwait(false);
            else
                await ReplyErrorLocalized("take_fail", n(amount) + CurrencySign, Format.Bold(user.ToString()), CurrencyPluralName).ConfigureAwait(false);
        }
...
";
        ...
        public async Task Rps(RpsPick pick, ShmartNumber amount = default)
        {
            ...
            else
            {
                ...
                amount = 0;
                msg = GetText("rps_win", Context.Client.CurrentUser.Mention, getRpsPick(wizbotPick),
                    getRpsPick(pick));
            }
