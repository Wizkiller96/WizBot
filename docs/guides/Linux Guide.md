...
```
#####Part II
**If** you are running WizBot for the first time on your system and never had any *prerequisites* installed, Press `5` and `enter` key, then `y` when you see the following:
```
...
Once done,		
#####Part V
You should see the options again within the **`tmux` session** named `wizbot` *(remember this one)*			
...
from the options choose `4` to **Run WizBot with Auto Restart.**
...
**Done**, You now have your own **WizBot**.		
