...
`.permrole` `.pr` | Sets a role which can change permissions. Supply no parameters to see the current one. Default is 'WizBot'.  | `.pr role`
...
`.updates` `.changelogs` | Shows a list of changes to WizBot. | `.updates`
