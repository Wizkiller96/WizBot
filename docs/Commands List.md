...
`$slot` | Play WizBot slots. Max bet is 999. 3 seconds cooldown per user.  | `$slot 5`
...
`>heal` | Heals someone. Revives those who fainted. Costs a Cherry Blossoms  | `>heal @someone`
`>type` | Get the poketype of the target.  | `>type @someone`
`>settype` | Set your poketype. Costs a Cherry Blossoms. Provide no arguments to see a list of available types.  | `>settype fire` or `>settype`
...
`.updates` | Show the lastest changes to WizBot.  | `.updates`